...
public class MediaEncoderFilter extends Filter {
    ...
    @GenerateFieldPort(hasDefault = true, name = "pauseNresumeRecording")
    private boolean mPauseResumeRecording;
    @GenerateFieldPort(hasDefault = true, name = "recordingProfile")
    private CamcorderProfileEx mProfile;
    ...
    @GenerateFieldPort(hasDefault = true, name = "videoEncoderBitrate")
    ...
    public MediaEncoderFilter(String name) {
        ...
        this.mVideoEncoderBitrate = 2000000;
        this.mMaxFileSize = 0;
        this.mMaxDurationMs = 0;
        this.mTimeBetweenTimeLapseFrameCaptureUs = 0;
        this.mPauseResumeRecording = false;
        this.mRecordingActive = false;
        this.mTimestampNs = 0;
        this.mLastTimeLapseFrameRealTimestampNs = 0;
        this.mNumFramesEncoded = 0;
        this.mCaptureTimeLapse = false;
        this.mIsRecStart = false;
        this.mSourceRegion = new Quad(new Point(0.0f, 0.0f), new Point(RotateView.DEFAULT_TEXT_SCALE_X, 0.0f), new Point(0.0f, RotateView.DEFAULT_TEXT_SCALE_X), new Point(RotateView.DEFAULT_TEXT_SCALE_X, RotateView.DEFAULT_TEXT_SCALE_X));
    ...
    public void fieldPortValueUpdated(String name, FilterContext context) {
        ...
        if (!name.equals("recording")) {
            ...
            } else if (name.equals("pauseNresumeRecording") && this.mIsRecStart) {
                if (this.mLogVerbose) {
                    Log.v(TAG, "pauseNresumeRecording = " + this.mPauseResumeRecording);
                }
                if (this.mPauseResumeRecording) {
                    if (this.mLogVerbose) {
                        Log.v(TAG, "MediaEncoderFilter call mMediaRecorder.pause()");
                    }
                    this.mMediaRecorder.pause();
                    return;
                }
                if (this.mLogVerbose) {
                    Log.v(TAG, "MediaEncoderFilter call mMediaRecorder.resume()");
                }
    ...
    private void updateMediaRecorderParams() {
        ...
        if (!(this.mCaptureTimeLapse || this.mAudioSource == NO_AUDIO_SOURCE)) {
            this.mMediaRecorder.setAudioSource(this.mAudioSource);
        }
        if (this.mProfile != null) {
            this.mFps = this.mProfile.videoFrameRate;
            if (this.mAudioSource != NO_AUDIO_SOURCE) {
                ...
                this.mMediaRecorder.setVideoEncodingBitRate(this.mVideoEncoderBitrate);
    ...
    private void startRecording(FilterContext context) {
        ...
        if (this.mWidth <= 0 || this.mHeight <= 0) {
            widthHeightSpecified = false;
        } else {
            widthHeightSpecified = true;
        }
        ...
        this.mScreen = (GLFrame) context.getFrameManager().newBoundFrame(screenFormat, DialogCreater.DIALOG_ID_HELP_HDR, 0);
        this.mMediaRecorder = new MediaRecorderEx();
        updateMediaRecorderParams();
        try {
            this.mMediaRecorder.prepare();
            this.mIsRecStart = true;
            if (this.mLogVerbose) {
                Log.v(TAG, "MediaEncoderFilter set mPauseResumeRecording = false / mPauseResumeRecording = " + this.mPauseResumeRecording);
            }
