...
public class SwitcherLeverHorizon extends SwitcherLever implements OnTouchListener {
    private Drawable mModeCamDrawable;
    private Drawable mModeDrawable;
    private Drawable mModeVideoDrawable;

    public SwitcherLeverHorizon(Context context) {
        super(context);
    ...
    public SwitcherLeverHorizon(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mModeDrawable = null;
        this.mModeCamDrawable = null;
        this.mModeVideoDrawable = null;
    ...
    public SwitcherLeverHorizon(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void trackTouchEvent(MotionEvent event) {
        ...
        int available = ((getWidth() - getPaddingStart()) - getPaddingEnd()) - drawableWidth;
        this.mPosition = ((((int) event.getX()) - getPaddingStart()) - getPaddingEnd()) - (drawableWidth / 2);
        if (this.mPosition < 0) {
            this.mPosition = 0;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawableWidth != 0 && drawableHeight != 0 && this.mRotationInfo != null) {
            if (this.mAnimationParkingStartTime != -1) {
                setAnimationStartTime(drawableWidth);
            }
            int offsetLeft = getPaddingStart() + this.mPosition;
    ...
    protected int getTouchPositionAvailable() {
        return ((getWidth() - getPaddingStart()) - getPaddingEnd()) - getDrawable().getIntrinsicWidth();
    }

    protected int getAniPositionAvailable(int drawablePosition) {
        return ((getWidth() - getPaddingStart()) - getPaddingEnd()) - drawablePosition;
    }
}
